...
HostedPlugin::~HostedPlugin() { close(); }

void HostedPlugin::report_startup_phase(StartupErrorCode phase) noexcept
{
    if (startup_phase_sink_)
        startup_phase_sink_->publish(phase);
}

bool HostedPlugin::configure_buses(std::uint32_t channels, std::string& error)
{
    main_input_bus_ = -1;
    main_output_bus_ = -1;
...
bool HostedPlugin::activate_configured_buses(std::string& error)
{
...
bool HostedPlugin::enumerate_parameters(std::string& error)
{
    parameters_.clear();
    if (!controller_)
        return true;
...
bool HostedPlugin::collect_io_layout_candidate(IoLayout& layout) noexcept
{
    layout = {};
    if (!component_ || !processor_)
        return false;
...
bool HostedPlugin::io_stop_processing() noexcept
{
    return processor_ && processor_->setProcessing(false) == kResultTrue;
}

bool HostedPlugin::io_deactivate() noexcept
{
    return component_ && component_->setActive(false) == kResultTrue;
}

bool HostedPlugin::io_inspect_requested_layout(IoLayout& layout) noexcept
{
    return collect_io_layout_candidate(layout);
}

IoArrangementResult HostedPlugin::io_confirm_requested_layout(const IoLayout&) noexcept
{
...
bool HostedPlugin::io_inspect_confirmed_layout(IoLayout& layout) noexcept
{
    return collect_io_layout_candidate(layout);
}

bool HostedPlugin::io_rebuild_processing(const IoLayout& layout) noexcept
{
...
bool HostedPlugin::io_activate() noexcept
{
    return component_ && component_->setActive(true) == kResultTrue;
}

bool HostedPlugin::io_query_latency(std::uint32_t& latency_samples) noexcept
{
    if (!processor_)
        return false;
...
bool HostedPlugin::io_start_processing() noexcept
{
    return processor_ && processor_->setProcessing(true) == kResultTrue;
}

void HostedPlugin::io_commit_layout(const IoLayout& layout,
...
bool HostedPlugin::set_processing(bool enabled) noexcept
{
    return processor_ && processor_->setProcessing(enabled) == kResultTrue;
}

bool HostedPlugin::set_active(bool enabled) noexcept
{
    return component_ && component_->setActive(enabled) == kResultTrue;
}

std::uint32_t HostedPlugin::get_latency_samples() noexcept
{
    return processor_ ? processor_->getLatencySamples() : 0;
}

void HostedPlugin::set_component_handler(IComponentHandler* handler) noexcept
{
    if (controller_)
...
EngineParameter* HostedPlugin::find_parameter(std::uint32_t id) noexcept
{
...
bool HostedPlugin::queue_parameter_impl(std::uint32_t id, double normalized, bool update_controller) noexcept
{
    if (!processor_)
        return false;
...
bool HostedPlugin::apply_pending_parameter_changes(ProcessData& data) noexcept
{
...
void HostedPlugin::finish_parameter_changes() noexcept
{
    input_parameter_changes_.clearQueue();
    parameter_changes_pending_ = false;
}

void HostedPlugin::record_parameter_update(std::uint32_t id, double normalized) noexcept
{
...
void HostedPlugin::capture_output_parameter_changes() noexcept
{
...
std::size_t HostedPlugin::take_parameter_updates(EngineParameterUpdate* destination, std::size_t capacity) noexcept
{
