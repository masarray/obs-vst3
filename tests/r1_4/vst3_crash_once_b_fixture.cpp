...
namespace safevst3::r1_4_fixture_b {
using namespace Steinberg;
using namespace Steinberg::Vst;

static const FUID kProcessorUid(0xB1414002, 0x92D74B77, 0xA29D2F12, 0x01400002);
constexpr auto kPluginName = "SafeVST3 R1-4 CrashOnce B plus1";
constexpr SpeakerArrangement kArrangement = SpeakerArr::kStereo;

void crash_once_if_armed() noexcept
{
    wchar_t event_name[256]{};
    const DWORD size = GetEnvironmentVariableW(L"SAFEVST3_R14_CRASH_B_EVENT", event_name, 256);
    if (size == 0 || size >= 256)
        return;
    ...
    if (state == WAIT_TIMEOUT) {
        SetEvent(event);
        CloseHandle(event);
        TerminateProcess(GetCurrentProcess(), 0xB14B);
        return;
    }
    CloseHandle(event);
}

class CrashOnceB final : public AudioEffect {
public:
    static FUnknown* create_instance(void*) { return static_cast<IAudioProcessor*>(new CrashOnceB()); }
...
using namespace Steinberg;
using namespace Steinberg::Vst;
using safevst3::r1_4_fixture_b::CrashOnceB;
using safevst3::r1_4_fixture_b::kPluginName;
using safevst3::r1_4_fixture_b::kProcessorUid;
...
DEF_CLASS2(INLINE_UID_FROM_FUID(kProcessorUid), PClassInfo::kManyInstances, kVstAudioEffectClass,
           kPluginName, 0, "Fx", "1.0.0", kVstVersionString, CrashOnceB::create_instance)
END_FACTORY
