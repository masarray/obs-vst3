...
function Require-Text([string]$haystack, [string]$needle, [string]$description) {
    if (-not $haystack.Contains($needle)) {
        throw "P7 Rack Editor contract missing: $description"
    }
}

# Qualified base / typography / product workflow remains underneath P7.
...
# P5/P6 measurement and master-control behavior remains authoritative beneath P7.
...
Require-Text $p6 'draw_metal_fader' 'qualified P6 satin-metal fader'
...
# P7 screenshot-driven luxury pass: stable geometry, contained strips and premium surface.
Require-Text $p7 'kP7SlotHeight = 44.0f' 'tighter one-row Rack strips'
Require-Text $p7 'kP7SlotControlReserve = 146.0f' 'reserved right-side control gutter'
...
Require-Text $p7 'draw_precise_slot_identity' 'precise centered slot identity renderer'
Require-Text $p7 'render_p7_master_surface' 'aligned compact master surface'
Require-Text $p7 'render_p7_loudness' 'centered compact loudness surface'
Require-Text $p7 'rack-p7-slot-well' 'contained Rack well'
Require-Text $p7 'rack-p7-master-console' 'non-scrolling master console'
Require-Text $p7 'ImGuiWindowFlags_AlwaysVerticalScrollbar' 'reserved Rack scrollbar gutter prevents width jumps'
Require-Text $p7 'ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse' 'master/lane scrollbar suppression'
...
Require-Text $p7 '"LUFS-I"' 'broadcast loudness label'
Require-Text $p7 '"dBTP"' 'true-peak label'
if ($p7.Contains('"GR"') -or $p7.Contains('Gain Reduction')) {
    throw 'P7 regression: generic gain-reduction meter must not consume the main Rack surface'
}
...
if ($master.Contains('std::mutex') -or $master.Contains('condition_variable') -or
    ...
    throw 'P7 regression: master gain path must remain lock-free/allocation-free'
}
...
if ($meter.Contains('std::mutex') -or $meter.Contains('condition_variable')) {
    throw 'P7 regression: stereo telemetry must not introduce locks'
}
...
Require-Text $cmake 'rack_editor_p7_luxury.hpp' 'P7 source-local force include'
...
if (-not (Test-Path $iconSourcePath) -or -not (Test-Path $iconPath)) {
    throw 'P7 Rack companion icon sources are missing'
}

# Competitor references may guide hierarchy only; no branding/assets ship.
foreach ($source in @($p2, $p3, $p4, $p5, $p6, $p7)) {
    if ($source.Contains('Waves') -or $source.Contains('StudioRack')) {
        throw 'P7 regression: shipping Rack skin must not contain competitor branding/product names'
    }
}

Write-Host 'P7 luxury polish + stable geometry + no-dim topology transition contract: PASS'