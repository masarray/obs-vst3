...
namespace safevst3::r1_3_fixture_c {
using namespace Steinberg;
using namespace Steinberg::Vst;

static const FUID kProcessorUid(0xC1313003, 0x92D74B77, 0xA29D2F12, 0x01300003);
constexpr auto kPluginName = "SafeVST3 R1-3 Affine C x3 slow-load";
constexpr SpeakerArrangement kArrangement = SpeakerArr::kStereo;

void signal_entered_load() noexcept
{
    wchar_t event_name[256]{};
    const DWORD size = GetEnvironmentVariableW(L"SAFEVST3_R13_C_LOAD_EVENT", event_name, 256);
    if (size == 0 || size >= 256)
        return;
...
class AffineC final : public AudioEffect {
public:
    static FUnknown* create_instance(void*) { return static_cast<IAudioProcessor*>(new AffineC()); }
    ...
    uint32 PLUGIN_API getLatencySamples() override { return 23; }
...
using namespace Steinberg;
using namespace Steinberg::Vst;
using safevst3::r1_3_fixture_c::AffineC;
using safevst3::r1_3_fixture_c::kPluginName;
using safevst3::r1_3_fixture_c::kProcessorUid;
...
DEF_CLASS2(INLINE_UID_FROM_FUID(kProcessorUid), PClassInfo::kManyInstances, kVstAudioEffectClass,
           kPluginName, 0, "Fx", "1.0.0", kVstVersionString, AffineC::create_instance)
END_FACTORY
